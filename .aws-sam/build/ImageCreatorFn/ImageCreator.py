...
logger.info("modelId is --> " + str(modelId))
if modelId is None:
...
def lambda_handler(event, context):
    ...
    if event.get('httpMethod') == 'POST':
        ...
        logger.info("contentType is --> " + str(contentType))
    else:
    ...
    if prompt is None:
        ...
        return buildResponse(400, 'No prompt found. A prompt must be provided to process the request')
    ...
    logger.info("img_time is --> " + str(img_time))
    image_name = 'image_' + str(img_time) + '.png'
    logger.info("image_name is --> " + str(image_name))
    ...
    try:
        ...
        logger.error('Error uploading image to target bucket: ' + str(e))
